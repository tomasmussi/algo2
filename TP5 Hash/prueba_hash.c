...
void prueba_hash_clave_null() {
	hash_t* hash = hash_crear(NULL);
	char *clave = "Clave null";
	print_test("Prueba hash insertar clave con NULL", hash_guardar(hash, clave, NULL));
	print_test("Prueba hash obtener valor NULL con clave", hash_obtener(hash,clave) == NULL);
	print_test("Prueba hash obtener valor NULL con clave", hash_pertenece(hash,clave));
	hash_destruir(hash);
}
