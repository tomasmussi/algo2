/*
 * prueba_abb.c
 * Pruebas para el tipo de dato abstracto Arbol Binario de Busqueda
...
void swap_c(char** valores, size_t i, size_t j) {
	char* aux = valores[i];
	valores[i] = valores[j];
	valores[j] = aux;
}

void swap_i(int** valores, size_t i, size_t j) {
	int* aux = valores[i];
	valores[i] = valores[j];
	valores[j] = aux;
}
...
void generar_parejas(char***c, int*** v, size_t largo) {
	...
	for (size_t i = largo - 1; i > 0; i--) {
		size_t pos = rand() % i;
		swap_c((char**)claves, pos, i);
		swap_i(valores, pos, i);
	}
...
void destruir_claves(char** claves, size_t largo) {
...
void prueba_abb_volumen(size_t largo, bool debug)
{
	...
	generar_parejas(&claves, &valores, largo);
...
void prueba_abb_iterar()
{
	...
	print_test("Prueba abb iterador ver actual, es la primera", strcmp(clave, claves[1]) == 0);
	...
	/* Segundo valor */
	clave = abb_iter_in_ver_actual(iter);
	print_test("Prueba abb iterador ver actual, es la del medio", strcmp(clave, claves[0]) == 0);
	...
	/* Tercer valor */
	clave = abb_iter_in_ver_actual(iter);
	print_test("Prueba abb iterador ver actual, es la ultima", strcmp(clave, claves[2]) == 0);
...
void prueba_abb_iterar_volumen(size_t largo)
{
	...
	for (i = 0; i < largo; i++) {
		...
		clave = abb_iter_in_ver_actual(iter);
		sprintf(clave_correcta, "%08d", i);
		if ( clave == NULL || strcmp(clave, clave_correcta) != 0) {
			ok = false;
			break;
		}
