...
/* Recorre la pila desde el primer elemento hasta el ultimo elemento apilado */
bool pila_iterar(pila_t *pila, bool (*visitar)(void *dato, void *extra), void *extra)
{
	bool seguir = true;
	int i = 0;
	while ((i < pila->cantidad) && seguir)
	{
		seguir = visitar(pila->datos[i] ,extra);
		//Como uso para verificar si un alumno ya esta inscripto
		//si lo encontre no tengo que modificar el valor de i para no
		//cambiar la condicion de retorno
		if (seguir)
			i++;
	}
	return pila->cantidad == i;
}
