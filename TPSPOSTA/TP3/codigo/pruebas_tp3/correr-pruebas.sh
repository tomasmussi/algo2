...
do
	...
	arg1=`grep '^#' "$f" | cut -d " " -f 1 | cut -c 1 --complement`
	arg2=`grep '^#' "$f" | cut -d " " -f 2`
	arg3=`grep '^#' "$f" | cut -d " " -f 3`
	arg4=`grep '^#' "$f" | cut -d " " -f 4`
	arg5=`grep '^#' "$f" | cut -d " " -f 5`
	cat "$f" | grep '^<' | cut -c 1 --complement >"$tmp_entrada_test"
	cat "$f" | grep '^>' | cut -c 1 --complement >"$tmp_salida_test"

	if [ $CORRER_VALGRIND == 1 ]; then
		valgrind --leak-check=full --show-reachable=yes -q --log-file="$tmp_valgrind" "$PROGRAMA" "$arg1" "$arg2" "$arg3" "$arg4" "$arg5" <"$tmp_entrada_test" >"$tmp_salida" 2>"$tmp_stderr"
	else
		"$PROGRAMA" "$arg1" "$arg2" "$ROOT/$arg3" "$ROOT/$arg4" "$ROOT/$arg5"  <"$tmp_entrada_test" >"$tmp_salida" 2>"$tmp_stderr"
	fi
